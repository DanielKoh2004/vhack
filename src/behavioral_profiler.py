...
class BehavioralProfiler:
    ...
    def _score_rapid_session(self, df):
        if all(c in df.columns for c in ['failed_login_attempts', 'tx_count_24h']):
            # Velocity signal
            velocity = df['failed_login_attempts'] * 0.5 + (df['tx_count_24h'] > 5).astype(int) * 0.5
    ...
    def predict(self, df):
        ...
        for idx in df.index:
            ...
            if scores.loc[idx, 'r_dev'] > 0.1:
                row_reasons.append("Amount significantly exceeds user's 30-day average")
            if scores.loc[idx, 'r_ctx'] > 0.1:
                row_reasons.append("High context risk (Foreign IP / New Device)")
...
if __name__ == "__main__":
    ...
    df = preprocessor.load_data(nrows=5)
    df_eng = FeatureEngineer().transform(df)
    
    profiler = BehavioralProfiler()
    scores, reasons = profiler.predict(df_eng)
    
    for i in range(len(scores)):
        print(f"Row {i} | Score: {scores[i]:.3f} | Reason: {reasons[i]}")
